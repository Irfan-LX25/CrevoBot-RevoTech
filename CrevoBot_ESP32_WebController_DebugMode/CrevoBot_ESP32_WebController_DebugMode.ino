...
<head>
  ...
  <style>
    ...
    .btn-stop { 
      background: #f39c12; border-radius: 12px; /* Kotak */
...
<script>
  ...
  function toggleFullScreen() {
    ...
    if(!doc.fullscreenElement && requestFullScreen) {
      ...
      document.getElementById('fs-btn').style.display = 'none'; // Sembunyikan tombol setelah aktif
    }
  }
  var lastRequest = 0;
  ...
  function move(dir, btn) {
    ...
    log("MOVE " + dir);
    sendReq("/move?dir=" + dir, true); // True = prioritas tinggi
  }
  function stop(btn) {
...
void moveRobot(String dir) {
  Serial.printf("ESP: GERAK %s (Speed: %d)\n", dir.c_str(), motorSpeed);
...
void setup() {
  ...
  Serial.println("\n--- ROBOT READY ---");
  Serial.print("IP Webserver: "); Serial.println(WiFi.softAPIP());
  ...
  server.on("/kick", HTTP_GET, [](AsyncWebServerRequest *request){
    if (request->hasParam("state")) {
      ...
      if(state == "1") {
        ...
        Serial.printf("ESP: KICK ON (Power: %d)\n", kickerPower);
      } else {
        ledcWrite(pwmChannelKick, 0);
        Serial.println("ESP: KICK OFF");
      }
    }
    request->send(200, "text/plain", "OK");
  });
  ...
  server.on("/setKickPower", HTTP_GET, [](AsyncWebServerRequest *request){
    if (request->hasParam("val")) {
      ...
      Serial.printf("ESP: Set Kick Power -> %d\n", kickerPower);
    }
    request->send(200, "text/plain", "OK");
  });
  server.begin();
